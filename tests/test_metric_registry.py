...
def test_register_adds_key():
    reg = MetricRegistry()
    reg.register("student/loss/train", object())
    assert "student/loss/train" in reg
    assert len(reg) == 1
...
def test_getitem_unknown_raises_key_error_with_suggestion():
    reg = MetricRegistry()
    reg.register("student/loss/train", object())
    with pytest.raises(KeyError, match="Did you mean.*student/loss/train"):
        reg["student/loss/trai"]  # noqa: B015
